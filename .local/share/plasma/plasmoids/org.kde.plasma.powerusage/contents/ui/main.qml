...
PlasmoidItem {
  ...
  onSampleIntervalSecondsChanged: {
    ...
    sampleBuffer = []
    var minRefresh = (sampleIntervalSeconds <= 1) ? 1 : (sampleIntervalSeconds * 2)
    if (refreshSeconds < minRefresh) {
  ...
  fullRepresentation: Item {
    ...
    ColumnLayout {
      ...
      GridLayout {
        ...
        rowSpacing: 8
        Layout.alignment: Qt.AlignHCenter
        property int inputColumnWidth: Math.max(sampleSpin.implicitWidth, refreshSpin.implicitWidth, fontSizeSpin.implicitWidth, paddingSpin.implicitWidth)
        ...
        PlasmaComponents.Label {
          ...
          ToolTip.visible: sampleRateHover.containsMouse
          ToolTip.delay: 300
          ToolTip.text: "How often a power reading is taken"
          MouseArea { id: sampleRateHover; anchors.fill: parent; hoverEnabled: true; acceptedButtons: Qt.NoButton }
        }
        SpinBox {
          id: sampleSpin
          ...
          Layout.preferredWidth: settingsGrid.inputColumnWidth
        }

        // Row 2: Display update interval
        PlasmaComponents.Label {
          id: refreshLabel
          text: "Display update interval (s)"
          Layout.alignment: Qt.AlignLeft | Qt.AlignVCenter
          ToolTip.visible: refreshHover.containsMouse
          ToolTip.delay: 300
          ToolTip.text: "Duration over which readings are averaged and updated in the UI"
          MouseArea { id: refreshHover; anchors.fill: parent; hoverEnabled: true; acceptedButtons: Qt.NoButton }
        }
        SpinBox {
          id: refreshSpin
          from: (root.sampleIntervalSeconds <= 1) ? 1 : (root.sampleIntervalSeconds * 2); to: 300; stepSize: 1
        ...
        // Row 5: Hide while charging
        CheckBox {
          text: "hide while charging"
